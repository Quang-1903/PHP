            ...
            <div class="container">
									...
									<ul class="nav-accordion">
										<li class="has-sub">
											...
											<ul class="toggle-content">
												<li><a href="left_sidebar.php"><span>All Products </span></a></li>
												<li>
													<?php 
                ...
                <!-- Popup product quick view -->
                     ...
                     if(isset($_GET['search']) && $_GET['search']!=''){
                        ...
                        {
                     	$sql='select * FROM sanpham WHERE TenSP like "%'.$_GET['search'].'%"  ORDER BY GiaBan '.$_GET['order'].' limit '.$perRow.','.$rowperPage.'';  
                     	$sqlcount='select * FROM sanpham WHERE TenSP like "%'.$_GET['search'].'%"  ORDER BY GiaBan '.$_GET['order'].'';
                     	$urlcurrent = 'left_sidebar.php?search='.$_GET['search'].'&page=';
                     	            
                        }
                        else
                        {
                        $sql='select * FROM sanpham WHERE TenSP like "%'.$_GET['search'].'%" limit '.$perRow.','.$rowperPage.'';  
                        $sqlcount='select * FROM sanpham WHERE TenSP like "%'.$_GET['search'].'%"';
                     }
                 }
                     //OKE theo giới tính theo loại
                     elseif(isset($_GET['gioitinh']) && $_GET['gioitinh']!='' && isset($_GET['loai']) && $_GET['loai']!=''){
                        if(isset($_GET['order']) && $_GET['order']!='')
                        {
                     	$sql='select * FROM sanpham WHERE GioiTinh ="'.$_GET['gioitinh'].'" AND Loai="'.$_GET['loai'].'" ORDER BY GiaBan '.$_GET['order'].' limit '.$perRow.','.$rowperPage.'';
                     	$sqlcount='select * FROM sanpham WHERE GioiTinh ="'.$_GET['gioitinh'].'" AND Loai="'.$_GET['loai'].'" ORDER BY GiaBan '.$_GET['order'].'';
                        }
                        else
                        {
                        	$sql='select * FROM sanpham WHERE GioiTinh ="'.$_GET['gioitinh'].'" AND Loai="'.$_GET['loai'].'" limit '.$perRow.','.$rowperPage.'';
                        	$sqlcount='select * FROM sanpham WHERE GioiTinh ="'.$_GET['gioitinh'].'" AND Loai="'.$_GET['loai'].'" ';
                        }
                     }
                     ...
                     elseif(isset($_GET['gioitinh']) && $_GET['gioitinh']!=''){
                     	if(isset($_GET['order']) && $_GET['order']!='')
                     	{
                     		$sql='select * FROM sanpham WHERE GioiTinh = "'.$_GET['gioitinh'].'" ORDER BY GiaBan '.$_GET['order'].' limit '.$perRow.','.$rowperPage.'  ';
                     		$sqlcount='select * FROM sanpham WHERE GioiTinh = "'.$_GET['gioitinh'].'" ORDER BY GiaBan '.$_GET['order'].'';

                     	}
                     	else
                     	{
                     		$sql='select * FROM sanpham WHERE GioiTinh = "'.$_GET['gioitinh'].'" limit '.$perRow.','.$rowperPage.'';
                     		$sqlcount='select * FROM sanpham WHERE GioiTinh = "'.$_GET['gioitinh'].'"';
                     	}
                     ...
                     elseif (isset($_GET['loai']) && $_GET['loai']!='') {
                     	if(isset($_GET['order']) && $_GET['order']!='')
                     	{
                     		...
                     		';
                     		$sqlcount='select * FROM sanpham,loaisp WHERE loaisp.MaLoai=sanpham.Loai AND loaisp.Loai like "%'.$_GET['loai'].'%" ORDER BY GiaBan '.$_GET['order'].'';

                     	}
                     	else
                     	{
                     		$sql='select * FROM sanpham,loaisp WHERE loaisp.MaLoai=sanpham.Loai AND loaisp.Loai like "%'.$_GET['loai'].'%" limit '.$perRow.','.$rowperPage.' ';
                     		$sqlcount='select * FROM sanpham,loaisp WHERE loaisp.MaLoai=sanpham.Loai AND loaisp.Loai like "%'.$_GET['loai'].'%"';
                     	}
                     ...
                     elseif (isset($_GET['from']) && $_GET['from']!='' && isset($_GET['to']) && $_GET['to']!='') {
                     	if(isset($_GET['order']) && $_GET['order']!='')
                     	{
                     		$sql='select * FROM sanpham WHERE GiaBan BETWEEN '.$_GET['from'].' AND '.$_GET['to'].'ORDER BY GiaBan '.$_GET['order'].' limit '.$perRow.','.$rowperPage.'';
                     		$sqlcount='select * FROM sanpham WHERE GiaBan BETWEEN '.$_GET['from'].' AND '.$_GET['to'].'ORDER BY GiaBan '.$_GET['order'].'';
                     	}
                     	else
                     	{
                     		$sql='select * FROM sanpham WHERE GiaBan BETWEEN '.$_GET['from'].' AND '.$_GET['to'].' limit '.$perRow.','.$rowperPage.' ';
                     		$sqlcount='select * FROM sanpham WHERE GiaBan BETWEEN '.$_GET['from'].' AND '.$_GET['to'].'';
                     	}
                     }
                     //OKE sắp xếp 
                     else
                     {
                     	    if(isset($_GET['order']) && $_GET['order']!='')
                            {
	                        $sql = 'select * FROM sanpham  ORDER BY GiaBan '.$_GET['order'] .' limit '.$perRow.','.$rowperPage.'' ;
	                        $sqlcount = 'select * FROM sanpham  ORDER BY GiaBan '.$_GET['order'] .'';
                     ...
                     for($i=1;$i<=$totalPage;$i++)
                     {
                     	if($page==$i)
                     	{
                     		...
                     		if(str_contains($url, '?'))
                            {
                            	if(str_contains($url, '?page'))
                            	{
                            		$listPage.= '<li> <a href="'.$parts[0].'?page='.$i.'">'.$i.'</a></li>';
                            	}
                            	elseif(str_contains($url, '&page'))
                                {
    	                            $listPage.= '<li> <a href="'.$parts[0].'&page='.$i.'">'.$i.'</a></li>';
                        ...
                        if($page > 1)
                     	{
                     		...
                     		if($page==$totalPage)
                     		{
                     			$listPage =	'<li> <a href="'.$parts[0].'page='.$prepage.'">PREV</a></li>'."".$listPage;
                     		}
                     		else
                     		{
                     			if(str_contains($url, '?') && $totalPage > 1)
                     			{
                     			    if(str_contains($url, 'page'))
                     			        {
                     				        $listPage =	'<li> <a href="'.$parts[0].'page='.$prepage.'">PREV</a></li>'."".$listPage."".'<li> <a href="'.$parts[0].'page='.$nextpage.'">NEXT</a></li>';
                     			        }
                     			    else
                     			        {
                                            $listPage =	'<li> <a href="'.$parts[0].'page='.$prepage.'">PREV</a></li>'."".$listPage."".'<li> <a href="'.$parts[0].'&page='.$nextpage.'">NEXT</a></li>';
                     	  ...
                     	  //       else
                     			// {
                     			// 	$listPage =	$listPage."".'<li> <a href="'.$parts[0].'?page='.$nextpage.'">NEXT</a></li>';
                     			// }
